...
PostgresDatabaseManager.prototype.createDbOwnerIfNotExist = function() {
  return this.masterQuery_("DO $body$ BEGIN CREATE ROLE %I LOGIN PASSWORD %L; EXCEPTION WHEN others THEN RAISE NOTICE 'User exists, not re-creating'; END $body$;", [this.config.knex.connection.user, this.config.knex.connection.password])
};
...
PostgresDatabaseManager.prototype.createDb = function(databaseName) {
  ...
  _.each(collate, function(locale) {
    promise = promise.catch(function() {
      return self.masterQuery_("CREATE DATABASE %I OWNER = %I ENCODING = 'UTF-8' LC_COLLATE = %L TEMPLATE template0", [databaseName, owner, locale])
    });
  });
  return promise;
};
 ...
 * @Override
 */
PostgresDatabaseManager.prototype.truncateDb = function(databaseName, ignoreTables) {
...
PostgresDatabaseManager.prototype._updateTableNameCache = function(knex, config) {
  this.cachedTableNames_ = knex('pg_tables').select('tablename').where('schemaname', 'public').then(function (tables) {
    return _.without(_.map(tables, 'tablename'), config.knex.migrations.tableName);
  });
};
