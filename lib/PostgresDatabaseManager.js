...
PostgresDatabaseManager.prototype._updateIdSequenceCache = function(knex, config) {
  ...
  // This is required for searching the minimum constraint for the sequence.
    ...
    var query = _.map(idTableNames, function (tableName) {
      return escape("SELECT '%I' AS table, substring(pg_get_serial_sequence('\"%I\"', 'id') from '\"\\?([^\".]+)\"\\?$') AS sequence, substring(pg_get_serial_sequence('\"%I\"', 'id') from '^\"\\?([^\".]+)\"\\?\.') AS schema",
  ...
  // Find min constraint for each of the id sequences.
    ...
    var query = _.map(result.rows, function (row) {
      return escape("SELECT '%I' AS table, '%s' AS sequence, minimum_value AS min FROM information_schema.sequences where sequence_name = '%s' and sequence_schema = '%s'",
                    row.table, row.sequence, row.sequence, row.schema);
    });
